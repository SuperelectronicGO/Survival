...
public class MapGenerator : MonoBehaviour
{
	...
	public enum DrawMode { NoiseMap, Mesh, FalloffMap , TopLeftGradientMap , TopRightGradientMap , BottomLeftGradientMap , BottomRightGradientMap , TopGradientMap , BottomGradientMap , LeftGradientMap , RightGradientMap };
	...
	MapData GenerateMapData(Vector2 centre)
	{
		...
		foreach(MRange ranger in terrainData.mRanges)
        {
            ...
            if ((cx >= ranger.originX-ranger.rangeSize) && (cx <= ranger.originX+ranger.rangeSize)&& (cy >= ranger.originY - ranger.rangeSize) && (cy <= ranger.originY + ranger.rangeSize))
            {
                if (ranger.indent)
                {
					for (int y = 0; y < mapChunkSize + 2; y++)
